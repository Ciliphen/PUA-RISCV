...
a 的值只会在 clock 上跳沿来时发生变化，当 clock 不变时，a 的值也不会发生变化。
...
这串代码实现的功能是，每个 clk 上跳沿来时，a 和 b 的值会互相交换；假如 a 和 b 分别为 0 和 1，当 clk 上跳沿来时，a、b 会被赋值为 1 和 0；
...
此时右下角会弹出这样的选项框，我们点击 Yes。如果没弹出来，我们可以重启 vscode 试试。
